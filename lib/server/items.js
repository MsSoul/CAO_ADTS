...
// Fetch distributed items for an employee
router.get("/:emp_id", async (req, res) => {
    ...
    try {
        const query = `
            SELECT di.*, i.ID AS ITEM_ID
            FROM distributed_items di 
            JOIN items i ON di.ITEM_ID = i.ID 
            WHERE di.accountable_emp = ? AND di.deleted = 0;
        `;

        const [distributedItems] = await db.query(query, [emp_id]);
        const itemIds = distributedItems.map(item => item.ITEM_ID);

        const itemDetails = await getItemDetails(itemIds);
        const itemsMap = Object.fromEntries(itemDetails.map(item => [item.ITEM_ID, item]));

        const responseItems = distributedItems.map(di => ({
            ...di,
            ...itemsMap[di.ITEM_ID]
        }));
...
JOIN distributed_items di ON bt.distributed_item_id = di.ITEM_ID
JOIN items i ON di.ITEM_ID = i.ID
