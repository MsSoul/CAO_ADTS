...
router.post("/borrow", async (req, res) => {
    try {
        ...
        // Save notification
        const [notifInsertResult] = await db.query(
        ...
        const notificationId = notifInsertResult.insertId;
        console.log("✅ Notification inserted with ID:", notificationId);

        // ✅ Instead of fetching notification again, just build the object and generate message:
        const notification = {
            ID: notificationId,
        ...
        const message = `Borrow request for ${notification.ITEM_NAME} (qty: ${notification.QUANTITY}) has been created.`;
        // If you have generateMessage(notification), you can use that line instead:
        // const message = generateMessage(notification);

        await sendNotification({
            for_emp: owner_emp_id,
            message: message,
        ...
        res.json({ message: "Borrow transaction created and notification sent." });
