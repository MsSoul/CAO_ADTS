...
router.post("/borrow", async (req, res) => {
    try {
        ...
        const [insertResult] = await db.query(
            ...
            (distributed_item_id, borrower_emp_id, owner_emp_id, quantity, DPT_ID, createdAt, updatedAt, status, remarks) 
            VALUES (?, ?, ?, ?, ?, NOW(), NOW(), 2, 1)`,
            [distributed_item_id, borrower_emp_id, owner_emp_id, quantity, DPT_ID]
        );
