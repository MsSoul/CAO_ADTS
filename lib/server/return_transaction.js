...
router.post("/return", async (req, res) => {
    try {
        ...
        const borrowerName = borrowerResult.length > 0
            ? `${capitalizeName(borrowerResult[0].FIRSTNAME)} ${capitalizeName(borrowerResult[0].LASTNAME)}`
            : `Employee ${borrower_emp_id}`;
        ...
        // Get owner details from the transaction itself
        const [ownerResult] = await db.query("SELECT FIRSTNAME, LASTNAME FROM employee WHERE ID = ?", [transaction.owner_emp_id]);
        const ownerName = ownerResult.length > 0
            ? `${capitalizeName(ownerResult[0].FIRSTNAME)} ${capitalizeName(ownerResult[0].LASTNAME)}`
            : `Employee ${transaction.owner_emp_id}`;
        ...
        const notifications = [
            ...
            { message: ownerMessage, for_emp: transaction.owner_emp_id, transaction_id: transaction.ID },
