...
//update email only
// POST /request-otp-new-email
router.post("/request-otp-new-email", async (req, res) => {
  const { emp_id, new_email } = req.body;

  if (!emp_id || !new_email) {
    return res.status(400).json({ error: "Employee ID and new email are required." });
  }

  try {
    // Optional: Check if email is already used
    const [existingUser] = await db.query("SELECT emp_id FROM users WHERE email = ?", [new_email]);
    ...
    // Save OTP with the new email temporarily (to a separate table or existing one)
    await db.query(
      `INSERT INTO user_otps (emp_id, otp, email, expires_at) 
       VALUES (?, ?, ?, DATE_ADD(NOW(), INTERVAL 5 MINUTE))`,
      [emp_id, otpCode, new_email]
    );

    // Send OTP to the new email
    await sendOtpEmail(new_email, otpCode);

    return res.json({ message: "OTP sent to the new email." });
  } catch (err) {
    console.error("Error in request-otp-new-email:", err.message);
...
router.post("/otp-for-update", async (req, res) => { //
  ...
  try {
    // 1. Fetch the user email based on emp_id
    const [userRows] = await db.query("SELECT email FROM users WHERE emp_id = ?", [emp_id]);
    
    if (userRows.length === 0) {
      return res.status(400).json({ error: "Employee ID not found." });
    }

    const email = userRows[0].email;  // Extract email from query result
    ...
    console.error("Error in otp-for-update:", err.message);
    return res.status(500).json({ error: "Server error." });
  }
});
