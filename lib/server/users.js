...
router.post(
  ...
  async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      return res
        .status(400)
        .json({ msg: "Please provide both ID Number and password." });
    }
    ...
    try {
      ...
      if (!user.email || !user.email.includes("@")) {
        console.log(`Invalid or missing email for emp_id: ${emp_Id}`);
        return res.status(400).json({
          msg: "Your email is missing or invalid. Please contact your administrator.",
        });
      }
